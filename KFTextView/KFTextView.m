...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _content = [[KFTextViewContent alloc]init];
...
- (void)setWithContent:(KFTextViewContent *)content{
...
- (void)setViewHeight:(CGFloat)height{
    ...
    if ([self.KFDelegate respondsToSelector:@selector(textView:heightWillChange:)]) {
        [self.KFDelegate textView:self heightWillChange:height];
    }
...
- (void)setTags:(NSArray *)tags{
    if (![tags.firstObject isKindOfClass:[KFTag class]]) {
        return;
    }
    for (KFTag *tag in tags) {
        [self setTag:tag];
    }
}
...
- (void)addTagToCurrentLocation:(NSString *)tagName{
    ...
    KFTag *newTag = [[KFTag alloc]initWithText:tagName andLocation:selectedRange.location];
